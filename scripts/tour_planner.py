...
class tour_planner():
    ...
    def greedy_agent(self, time_limit = 1000 , std_dev = 3, human_num = 5, human_choice = 5, flag_greedy_in_time = True, human_demand_bool = [], human_demand_int_unique= []):
        ...
        if(flag_greedy_in_time):
            ...
            max_time_to_get_back = max(edge_time[demanded_pois,-2]) + 2*node_time[5]
            total_time = 0.0
            route_time_list = []
            ...
            demanded_pois = list(np.argsort(unsorted_demanded_pois))
            demanded_pois.reverse()
            route_list = []
            current_poi_index = place_num
            route_list.append(current_poi_index)
...
def main():
    ...
    save_results(full_result_greedy_demand, "greedy_in_demand")
